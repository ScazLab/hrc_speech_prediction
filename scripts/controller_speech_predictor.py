...
from trainSpeechPredictor import (left_obj_dict, right_obj_dict, id_dict,
                                  left_aruco_topic, right_aruco_topic,
                                  left_state_topic, right_state_topic,
                                  speech_topic,
                                  VOCAB_PATH, MODEL_PATH, BAG_DIR_PATH,
                                  create_vocab, create_data_set)
...
def createStateAndSpeechVec():
    ...
    while counter < 10:
        ...
        right_aruco_msg = rospy.wait_for_message(right_aruco_topic, MarkerArray).markers
        for o in left_aruco_msg:
            left_dict[o.id] += 1
        for o in right_aruco_msg:
            right_dict[o.id] += 1
        counter += 1
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        ...
        if left_state == "START" or left_state == "DONE":
            left_home = True
        if right_state == "START" or right_state == "DONE":
            right_home = True

        print("{}, {}".format(left_state, right_state))
